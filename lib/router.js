...
Router.prototype.resource = function( controller ) {
    ...
    return [
      this.get('/'+controller_slug+'(.:format)', 'GET').to(controller+'.index'),
      this.get('/'+controller_slug+'/add(.:format)', 'GET').to(controller+'.add'),
      this.get('/'+controller_slug+'/:id(.:format)', 'GET').to(controller+'.show'),
      this.get('/'+controller_slug+'/:id/edit(.:format)', 'GET').to(controller+'.edit'),
      this.post('/'+controller_slug+'(.:format)', 'POST').to(controller+'.create'),
      this.put('/'+controller_slug+'/:id(.:format)', 'PUT').to(controller+'.update'),
      this.del('/'+controller_slug+'/:id(.:format)', 'DELETE').to(controller+'.destroy')
    ];
  };
...
Router.prototype.all = function( path, method ) {
    ...
    for (var i in this.routes) {
      params = this.routes[i].parse(path, method)  // TODO: use call or apply (I forget which) to make 'method' optional
